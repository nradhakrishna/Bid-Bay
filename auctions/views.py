...
llm = ChatTogether(
    ...
    together_api_key="ba84d330b8f8dd322022ec3b1dfc5c1c9595736de8e2ca46bfc8944fd31f28ca"
)
prompt = ChatPromptTemplate(
    messages=[
        SystemMessagePromptTemplate.from_template(
            "You are a nice bidding website chatbot having a conversation with a human.The website has the following features users can bid on items,upload items for bidding using create listing feature, add items to users watchlist, and leave comments on items. The chatbot should be able to answer questions about the website and help users with their bidding experience."
        ),
...
conversation = LLMChain(
    llm=llm,
    prompt=prompt,
    verbose=False,#true to see what it runs
    memory=memory,
)
def ask(question):
